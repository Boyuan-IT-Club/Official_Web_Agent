...
def client(monkeypatch: pytest.MonkeyPatch, web_no_real_pg: None) -> TestClient:
    # web_no_real_pg(tests/conftest.py):lifespan 的建表自举/启动恢复与路由侧
    # 审计都不落真 PG。无 PG 时它们抛错 → catch 把 app.state.checkpointer 置
    # None → 恢复路径 503,checkpointer stub 形同虚设(CI 实测无 PG 必挂)。
